...
from .utils_pandaset import load_info, load_conditions, load_manifest, load_bin_info
...
class DatasetPandaSet(Dataset):
    ...
    camera_map = dict(zip(camera_types, ["front_camera", "front_right_camera", "front_left_camera",
                                         "back_camera", "left_camera", "right_camera"]))

    def __init__(self, cfg: DatasetPandaSetCfg, stage: Stage, view_sampler: ViewSampler,
        ...
        if not 0 < cfg.near < cfg.far:
            raise ValueError("PandaSet requires 0 < near < far")
        ...
        self.manifest, self.bin_tokens = load_manifest(
            self.processed_root, "pandaset", self.camera_map, storage_split)
